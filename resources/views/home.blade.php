...
<body class="bg-white">
    ...
    <header class="relative w-full h-[450px] md:h-[500px]">
        <!-- ... (konten header) ... -->
        <img src="{{ asset('storage/home/banner.jpg') }}">

             alt="{{ $hero['alt_text'] ?? 'New Collection' }}" 
             class="w-full h-full object-cover">
        
        <div class="absolute inset-0 bg-black/30"></div>

        <div class="absolute top-1/4 left-1/2 -translate-x-1/2 text-center text-white">
            <h2 class="text-2xl md:text-4xl font-light">{{ $hero['title'] ?? 'New Brown Collection' }}</h2>
            <h1 class="text-5xl md:text-7xl font-bold my-2">{{ $hero['discount'] ?? '30% OFF' }}</h1>
    ...
    <main class="container mx-auto px-4 mt-8">
        ...
        <section class="grid grid-cols-2 md:grid-cols-4 grid-rows-2 gap-4 h-[600px] md:h-[500px]">
            ...
            <div class="col-span-1 row-span-2 rounded-lg overflow-hidden">
                <img src="{{ $promoGrid1['boots']['image_url'] ?? 'https://via.placeholder.com/300x600.png?text=Boots' }}" 
                     alt="Boots" class="w-full h-full object-cover">
            </div>
            <div class="col-span-1 md:col-span-2 row-span-1 rounded-lg overflow-hidden">
                <img src="{{ $promoGrid1['arrived']['image_url'] ?? 'https://via.placeholder.com/600x300.png?text=Arrived' }}" 
            ...
            <div class="col-span-1 row-span-1 flex flex-col gap-4">
                <div class="flex-1 rounded-lg overflow-hidden">
                    <img src="{{ $promoGrid1['coupon1']['image_url'] ?? 'https://via.placeholder.com/300x140.png?text=Coupon+1' }}" 
                         alt="Coupon 1" class="w-full h-full object-cover">
                </div>
                <div class="flex-1 rounded-lg overflow-hidden">
                     <img src="{{ $promoGrid1['coupon2']['image_url'] ?? 'https://via.placeholder.com/300x140.png?text=Coupon+2' }}" 
                         alt="Coupon 2" class="w-full h-full object-cover">
                </div>
            </div>
            <div class="col-span-1 md:col-span-2 row-span-1 rounded-lg overflow-hidden">
                <img src="{{ $promoGrid1['sale']['image_url'] ?? 'https://via.placeholder.com/600x300.png?text=70%25+OFF' }}" 
                     alt="Sale" class="w-full h-full object-cover">
            </div>
            <div class="col-span-1 row-span-1 rounded-lg overflow-hidden">
                <img src="{{ $promoGrid1['style']['image_url'] ?? 'https://via.placeholder.com/300x300.png?text=Make+Style' }}" 
        ...
        <section class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
            ...
            <div class="md:col-span-2 relative rounded-lg overflow-hidden h-[500px] md:h-auto">
                <img src="{{ $promoGrid2['heels']['image_url'] ?? 'https://via.placeholder.com/600x600.png?text=Heels' }}" 
                     alt="Heels" class="w-full h-full object-cover">
                <div class="absolute bottom-10 left-10 text-gray-800">
                    <h3 class="text-2xl font-serif italic">{{ $promoGrid2['heels']['title'] ?? 'Step into your fairytale' }}</h3>
                    <p class="text-lg font-serif">{{ $promoGrid2['heels']['subtitle'] ?? 'Every step is a whisper of elegance' }}</p>
                </div>
            </div>
            <div class="md:col-span-1 flex flex-col gap-4">
                <div class="rounded-lg overflow-hidden">
                    <img src="{{ $promoGrid2['flats']['image_url'] ?? 'https://via.placeholder.com/400x250.png?text=Flats' }}" 
                         alt="Flats" class="w-full h-full object-cover">
                </div>
                <div class="rounded-lg overflow-hidden">
                    <img src="{{ $promoGrid2['white_heels']['image_url'] ?? 'https://via.placeholder.com/400x250.png?text=White+Heels' }}" 
                         alt="White Heels" class="w-full h-full object-cover">
                </div>
                <div class="rounded-lg overflow-hidden">
                    <img src="{{ $promoGrid2['dunk']['image_url'] ?? 'https://via.placeholder.com/400x300.png?text=Dunk+Poster' }}" 
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                ...
                <div class="border border-gray-200 rounded-lg shadow-md overflow-hidden bg-gray-50">
                    ...
                    <div class="bg-gray-200 h-64">
                        <img src="{{ $product->image_url ?? 'https://via.placeholder.com/400x300.png?text=Product' }}" 
                             alt="{{ $product->name }}" 
                             class="w-full h-full object-cover">
                    </div>
                    <div class="p-4">
                        ...
                        <div class="flex items-center my-2">
                            ...
                            @endfor
                            <span class="ml-2 text-gray-500 text-sm">({{ $product->rating }})</span>
                        </div>
